package ru.senla.scooterrental.discount.service.impl;
...
public class DiscountServiceImpl implements DiscountService {
    ...
    public DiscountServiceImpl(
        ...
        this.promoCodeRepository = requireNonNull(
                promoCodeRepository,
    ...
    public PromoCode createPromoCode(PromoCode promoCode) {
        ...
        log.info(
                "Creating promo code: code={}",
                promoCode.getCode()
        );

        if (promoCodeRepository.existsByCode(
                promoCode.getCode()
        )) {

            log.warn(
                    "Promo code creation rejected: code={} already exists",
                    promoCode.getCode()
            );
    ...
    public PromoCode getByCodeOrThrow(String code) {
        ...
        return promoCodeRepository.findByCode(
                        normalizedCode
                )
                .orElseThrow(() ->
                        new DiscountValidationException(
                                "Промокод не найден"
                        )
                );
    }
    ...
    public PromoCode getPromoCodeById(Long promoCodeId) {
        ...
        return promoCodeRepository.findById(
                        promoCodeId
                )
                .orElseThrow(() ->
                        new DiscountValidationException(
    ...
    public BigDecimal applyDiscount(
        ...
        if (code == null) {
            ...
            log.info(
                    "Discount skipped: promo code is not provided, price={}",
                    validPrice
            );
        ...
        PromoCode promoCode =
                getByCodeOrThrow(normalizedCode);

        if (!promoCode.isActive()) {

            log.warn(
                    "Discount rejected: promo code is inactive, code={}",
                    normalizedCode
            );
    ...
    public void deactivate(String code) {
        ...
        log.info(
                "Promo code deactivated successfully: code={}",
                promoCode.getCode()
        );
    }
    ...
    public void activate(String code) {
        ...
        log.info(
                "Promo code activated successfully: code={}",
                promoCode.getCode()
        );
    }
